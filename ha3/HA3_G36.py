...
estimator = [DecisionTreeRegressor(max_depth=3), LinearRegression(), KNeighborsRegressor(n_neighbors=3), LinearSVR(dual='auto')]
...
mdl = AdaBoostRegressor (estimator=KNeighborsRegressor (n_neighbors=3), n_estimators=50, learning_rate=0.5, random_state=10)
mdl.fit(X_train_pca, y_train_new)
preds = mdl.predict (X_test_pca)
plt.scatter(y_test, preds, color = '#C17CD9')
plt.plot(np.linspace(0, 1.5, 100), np.linspace(0, 1.5, 100))
